...
Profiler.prototype.profhandler = function(req, res) {
  const profid = uuid.v4();
  res.header('Content-type', 'application/octet-stream');
  res.header('Content-disposition', `attachment; filename=profile-${profid}.cpuprofile`);
  const seconds = Math.min(30, Number(req.param('seconds'))) || 30;
  v8Profiler.startProfiling(profid);
  setTimeout(() => {
    const profile = v8Profiler.stopProfiling(profid);
    profile.export()
      .pipe(res)
      .on('finish', () => {
        profile.delete();
...
Profiler.prototype.heapHandler = function(req, res) {
  const snapid = uuid.v4();
  res.header('Content-type', 'application/octet-stream');
  res.header('Content-disposition', `attachment; filename=snapshot-${snapid}.heapsnapshot`);
  const snapshot = v8Profiler.takeSnapshot(snapid);
  snapshot.export()
    .pipe(res)
    .on('finish', () => {
      snapshot.delete();
...
Profiler.prototype.growthHandler = function(req, res) {
  const snapid = uuid.v4();
  const seconds = Math.min(30, Number(req.param('seconds'))) || 30;
  const snapshot = v8Profiler.takeSnapshot(snapid);
  setTimeout(() => {
    const snapshot2 = v8Profiler.takeSnapshot(snapid + '-compare');
